...
**背景**:
MyTask 需要为四个角色 (Community, Taskor, Supplier, Jury) 提供 AI 驱动的决策辅助。传统的单一代理系统无法处理复杂的多角色协调与风险管理。
...
**背景**:
MyTask 的四个角色需要实时协调：Community 的成本决策影响 Taskor 的报价，而 Supplier 的定价影响两者。这需要一个能处理循环依赖的图形编排系统。
